...
/* allow 2 second wait time */
#define DEVICE_WAIT_TIMEOUT     (2*HZ)
...
{
    struct btusb_miscdevice *misc_dev = (struct btusb_miscdevice *)(file->private_data);
    struct btusb *p_dev;
    int idx;

    BTUSB_INFO("enter\n");

    if (unlikely(!misc_dev))
    {
        BTUSB_ERR("misc device is null\n");
        return -ENODEV;
    }

    mutex_lock(&misc_dev->dev_lock);
    p_dev = misc_dev->dev;
    BTUSB_INFO("p_dev=%p\n", p_dev);
    if (unlikely(!p_dev))
    {
        BTUSB_ERR("can't find device\n");
        mutex_unlock(&misc_dev->dev_lock);
        return -ENODEV;
    }

    /* acquire usage count so we can do what we need to do */
    kref_get(&p_dev->kref);
    mutex_unlock(&misc_dev->dev_lock);
    ...
    wake_up_interruptible(&misc_dev->rx_wait_q);
...
/*******************************************************************************
 **
 ** Function         btusb_miscdevice_get_rx_packet
 **
 ** Description      Calls btusb_get_rx_packet internally but performs locking
 **
 *******************************************************************************/
static BT_HDR *btusb_miscdevice_get_rx_packet(struct btusb_miscdevice *p_misc_dev)
{
    BT_HDR *header = NULL;

    mutex_lock(&p_misc_dev->dev_lock);
    if (p_misc_dev->dev)
    {
        /* call to get the next buffer */
        header = btusb_get_rx_packet(p_misc_dev->dev);
    }
    mutex_unlock(&p_misc_dev->dev_lock);
...
retry:
    ...
    if (unlikely(!p_dev))
    {
        ...
        if (retry &&
            wait_event_interruptible_timeout(misc_dev->rx_wait_q,
                                             (misc_dev->dev),
                                             DEVICE_WAIT_TIMEOUT) > 0)
        {
            retry = 0;
            goto retry;
        }
    ...
    if (unlikely(!p_dev->p_main_intf))
    {
        ...
        if (retry &&
            wait_event_interruptible_timeout(misc_dev->rx_wait_q,
                                             (misc_dev->dev && misc_dev->dev->p_main_intf),
                                             DEVICE_WAIT_TIMEOUT) > 0)
        {
            retry = 0;
            goto retry;
    ...
    /* wait for an event */
    retval = wait_event_interruptible(misc_dev->rx_wait_q, btusb_miscdevice_get_rx_packet(misc_dev));
    if (unlikely(retval))
    {
    ...
    /* device handle may have changed */
    p_dev = misc_dev->dev;
...
unsigned int btusb_poll(struct file *file, struct poll_table_struct *p_pt)
{
    ...
    poll_wait(file, &misc_dev->rx_wait_q, p_pt);
...
retry:
    ...
    switch (cmd)
    {
        ...
        case IOCTL_BTWUSB_ADD_VOICE_CHANNEL:
            ...
            retval = btusb_add_voice_channel(p_dev, sco_info.handle, sco_info.burst);
        ...
        case IOCTL_BTWUSB_REMOVE_VOICE_CHANNEL:
            ...
            retval = btusb_remove_voice_channel(p_dev, sco_info.handle);
        ...
        case TIOCMIWAIT:
        {
            ...
            while (1)
            {
                ...
                add_wait_queue(&misc_dev->rx_wait_q, &wait);
                set_current_state(TASK_INTERRUPTIBLE);
                schedule();
                BTUSB_DBG("removing task from wait list\n");
                remove_wait_queue(&misc_dev->rx_wait_q, &wait);
