...
#define MIC_FEATURE_REPORT_ID 0xF8
...
static struct snd_card *atvr_snd_card = NULL;
static int dev;
...
static int snd_atvr_mic_control(struct snd_atvr *atvr_snd, int on)
{
	struct hid_device *hdev = atvr_snd->hdev;
	__u8 buf[2] = {MIC_FEATURE_REPORT_ID, on?0x02:0x03};
	int ret;

	if (!hdev) {
		pr_err("%s:%d - ERROR HID device not present\n",
			__func__, __LINE__);
		return -ENODEV;
	}
...
static int snd_atvr_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		...
		/* Turn on the MIC */
		if (snd_atvr_mic_control(atvr_snd, 1)) {
			snd_atvr_log("%s Unable to turn on MIC\n", __func__);
		}
...
static int atvr_snd_initialize(struct hid_device *hdev, struct snd_card **ret_card)
{
	struct snd_card *card;
	...
	u8 *data, int size)
{
	...
	if (report->id == ADPCM_AUDIO_REPORT_ID) {
		...
		if (data[2] & KEYCODE_PRESENT_IN_AUDIO_PACKET_FLAG) {
			u8 *key_data = hid_alloc_report_buf(report, GFP_KERNEL);
			if (!key_data) {
				pr_err("%s: failed to allocate report buffer\n",
					__func__);
				return 0;
			}
			...
			hid_report_raw_event(hdev, 0, key_data, 3, 0);
...
static int atvr_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	if (!atvr_snd_card) {
		ret = atvr_snd_initialize(hdev, &atvr_snd_card);
		if (ret)
			goto err_stop;
		switch_set_state(&h2w_switch, BIT_HEADSET);
	} else {
		struct snd_atvr *atvr_snd = atvr_snd_card->private_data;
		atvr_snd->hdev = hdev;
	}
